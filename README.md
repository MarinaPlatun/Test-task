...
Сервисы-генераторы generatorOne и generatorTwo по таймеру - 10 секунд для generatorOne и 17 секунд для generatorTwo - создают в базе данных объект событие (NEW), а затем отправляют сообщение о том, что событие создано, кто (какой сервис) является создателем.
Сервис-регистратор registrator с помощью пакетной обработки получает через кафку сообщения о событиях, создает в БД запись о событии, а затем отсылает ответное сообщение-подтверждение.
